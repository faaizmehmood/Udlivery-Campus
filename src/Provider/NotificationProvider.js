...
class NotificationProvider {
  ...
  notification_arr_schedule(notification_arr) {
    for (let i = 0; i < notification_arr.length; i++) {
      ...
      this.notificationfunctionSchedule(
        message,
        action_json,
        player_id,
        title,
  ...
  Chatnotificationfunction(massege, action_json, playerid, title, image) {
    //console.log('innotif')
    // console.log('player_id of Notification Provider', playerid);
    //console.log('action_json', action_json)
    ...
    fetch('https://onesignal.com/api/v1/notifications', {
      ...
      .then(obj => {
        // global.props.hideLoader();
        console.log("nototottottot sent", obj);
      })
      .catch(error => {
        console.log('One Signal Catch error: ====> ', error);
        // global.props.hideLoader();
        reject(error);
      });
  }
