...
export default class Chat extends Component {
  ...
  OpenDoc = name => {
    ...
    if (name != null) {
      ...
      Linking.openURL(docurl).catch(err =>
        Alert.alert('Please contact to admin'),
      );
    }
  };
  getmessagedata = async () => {
    ...
    if (inbox_count >= 0) {
      ...
      if (jsonData.name != 'NA') {
        ...
        this.setState({
          name: jsonData.name,
          online_status: jsonData.onlineStatus,
        });
    ...
    if (FirebaseInboxJson.length > 0) {
      var find_inbox_index2 = FirebaseInboxJson.findIndex(
        x => x.user_id == other_user_id,
      );
  ...
  sendNotificationSignle = async (
    ...
    consolepro.consolelog(
      'sendNotificationSignle user_id_member',
      user_id_member,
    );

    consolepro.consolelog('update delete_flag', user_id_member);
    consolepro.consolelog(
      'sendNotificationSignle FirebaseUserJson',
      FirebaseUserJson,
    );
    ...
    if (user_check_inbox >= 0) {
      consolepro.consolelog(
        'FirebaseUserJson subuser',
        FirebaseUserJson[user_check_inbox],
      );
      ...
      consolepro.consolelog(
        'chat_room_id_get',
        chat_room_id_get + '//' + chat_room_id_get,
      );
      consolepro.consolelog(
        'player_id_get',
        user_id_member + '//' + player_id_get,
      );
      ...
      if (notification_status == 1) {
        ...
        if (player_id_get != 'no' && player_id_get != '123456') {
          ...
          if (player_id_arr.length > 0) {
            ...
            notification.Chatnotificationfunction(
              message,
              action_json,
              player_id_get,
              title,
  ...
  show_user_message_chat = () => {
    ...
    consolepro.consolelog(
      'find_inbox_index chatshow_user_message_chat',
      find_inbox_index,
    );
    consolepro.consolelog(
      'other_user_id chatshow_user_message_chat',
      other_user_id,
    );
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: () => consolepro.consolelog('No Pressed'),
          style: 'cancel',
        },
      ],
      {cancelable: false},
  ...
  block_unblock_user_final = () => {
    ...
    firebaseprovider.blockuserfunction(
      this.state.user_id,
      other_user_id,
      status,
    );
                  ...
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <View
                      style={{
                        ...
                        marginRight:
                          config.textalign == 'left' ? 0 : (mobileW * 2) / 100,
                      }}>
                      <Text
                        style={{
                      ...
                      {this.state.online_status == 'true' && (
                        <Text
                          ...
                          {Lang_chg.online_txt[config.language]}
                        </Text>
                      )}
                    </View>
                  </View>
                          ...
                          <View
                            ...
                            {item.messageType == 'image' && (
                              <TouchableOpacity
                                ...
                                <Image
                                  source={{uri: config.img_url1 + item.name}}
                                  style={{
            ...
            {this.state.block_status == 'no' && (
              <View
                ...
                <TouchableOpacity
                  ...
                  <View
                    style={{width: '100%', marginLeft: (mobileW * 2) / 100}}>
                    <TextInput
                      onChangeText={txt => {
                      ...
                      onFocus={() => {
                        this.setState({newpad: false});
                      }}
                      onBlur={() => {
                        this.setState({newpad: true});
                      }}></TextInput>
                  </View>
                </TouchableOpacity>
                <TouchableOpacity
            ...
            {this.state.block_status == 'yes' && (
              <View
                ...
                <Text
                  ...
                  {Lang_chg.you_blocked_this_user[config.language]}
                </Text>
              </View>
            )}
          </View>
