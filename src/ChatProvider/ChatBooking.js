...
import { config, msgProvider, localStorage, apifuntion, msgText, msgTitle, consolepro, Lang_chg, Font, Colors, mobileH, mobileW, localimag, Cameragallery, mediaprovider, firebaseprovider } from '../Provider/utilslib/Utils';
import firebase from './Config1';
import Firebase from 'firebase/app';
import { ref, onValue, remove, get } from 'firebase/database';
import ImagePicker from 'react-native-image-crop-picker';
import DeviceInfo from 'react-native-device-info';
import { notification } from '../Provider/utilslib/Utils';
...
export default class Chat extends Component {
    ...
    async listenToMessages() {
        ...
        const messagesRef = ref(db, `message/${messageIdME}`);

        onValue(messagesRef, (snapshot) => {
            if (snapshot.exists()) {
                const data = snapshot.val();
                const dataObj = Object.values(data);
                this.setState({ data1: Object.values(dataObj) });
            } else {
    ...
    async setInboxStatus(status) {
        localStorage.setItemString('isInInbox', status);
    }

    async checkCurrentInbox() {
        let userData = await localStorage.getItemObject('user_arr');
        let data = this.props.route.params.chatdata;
        var user_id = userData.user_id;
        var other_user_id = data.other_user_id;
        var booking_id = data.booking_id;
        var messageId = 'u_' + user_id + '__u_' + other_user_id + '_b_' + booking_id;

        this.setInboxStatus(messageId);
    }

    componentDidMount() {
        this.checkCurrentInbox();
    ...
    componentDidUpdate(prevProps, prevState) {
        // Check if the state you're interested in has changed
        if (this.state.messages !== prevState.messages) {
            // Perform actions or side effects here
            console.log('State has changed:', this.state.messages);

            // You can call a function or make API requests, etc.
        }
    }
    ...
    sendmessagebtn = async () => {
        ...
        const tokenRef = ref(db, `token/u_${data?.other_user_id}`);
        const pushToken = await get(tokenRef);

        if (pushToken.exists()) {
            let pushTokenData = pushToken.val();

            const token = pushTokenData?.token;
            let userdata = await localStorage.getItemObject('user_arr');

            notification.Chatnotificationfunction(message, '', token, userdata?.name)
        }
    }
    sendmessagecallbtn = async (messageType, message) => {
        ...
        firebaseprovider.UpdateUserInboxOther(other_user_id_send, inbox_id_other, jsonUserDataOther);
        var count_new = 0;
        var query = ref('users/' + other_user_id_send + '/myInbox/' + inbox_id_other);
    ...
    show_user_message_chat = () => {
        ...
        var queryOff = ref('message/').child(userChatIdGlobal);
        ...
        userChatIdGlobal = userChatId;
        var query = ref('message/' + userChatId).orderByChild("timestamp");
    ...
    ClearChatConfirm = async () => {
        ...
        let chatRef = ref(db, `message/${messageIdME}`);
        remove(chatRef);
        // messagedata=[]
    ...
    btnOpneImageOption = (index) => {
        ...
        ImagePicker.openPicker(options).then((response) => {
            ...
            if (response.didCancel) {
                ...
                this.setState({
                    filePath: response,
                    fileData: response?.modificationDate,
                ...
                consolepro.consolelog('userId: ', user_id)
                consolepro.consolelog('url', url)
                consolepro.consolelog('data', data)
                consolepro.consolelog('respons Data: ', response)
                apifuntion.postApi(url, data).then(async (obj) => {
                    consolepro.consolelog('obj', obj);
                    if (obj.success == 'true') {
                        console.log('true asasdas')
                        this.setState({ bottom: 0, loading: false })
                        this.sendmessagecallbtn('image', obj.file)

                        let dataInfo = this.state.data;
                        const tokenRef = ref(db, `token/u_${dataInfo?.other_user_id}`);
                        const pushToken = await get(tokenRef);
                        ...
                        if (pushToken.exists()) {
                            ...
                            notification.Chatnotificationfunction('image', '', token, userdata?.name, config.img_url1 + obj.file)
                        }
                    }
                    else {
                        console.log(false)
            ...
            .catch((error) => {
                console.log('I am the catch', error)
    ...
    // From This My code is Starting Faaiz

    // Camerapopen = async (response) => {
    //     mediaprovider
    //         .launchCamera(true)
    //     // .launchCamera(false)
                    ...
                    <View style={{ flex: 1, backgroundColor: Colors.white_color }}>
                        <View style={{ paddingVertical: mobileH * 1 / 100, backgroundColor: Colors.theme_color }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: mobileW * 3 / 100, backgroundColor: Colors.theme_color }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        {console.log('imgggg=>', this.state.data.image)}
                                        <Image
                                            onError={() => {
                                                var data = this.state.data;
                                                data.image = null
                                        ...
                                        <View style={{ width: '97%', alignSelf: 'center', paddingVertical: 7 }}>
                                            {item.userid != this.state.user_id && <View style={{ alignSelf: 'flex-start', width: '70%' }}>
                                                <View style={{
                                                    ...
                                                    paddingVertical: 1.5, paddingHorizontal: 8, alignSelf: 'flex-start', borderTopStartRadius: 6, borderTopEndRadius: 6, borderBottomLeftRadius: 6
                                                }}>
                                                    {item.messageType == 'text' && <Text style={{ fontSize: 14, fontFamily: Font.FontBold, color: Colors.black_color, position: "relative", top: 8 }}>{item.message}</Text>}
                                                    {item.messageType == 'image' && <TouchableOpacity activeOpacity={0.9} onPress={() => { this.props.navigation.navigate('ViewImage', { 'image': item.message }) }}>
                                                        <Image source={{ uri: config.img_url1 + item.message }} style={{ width: mobileW * 42 / 100, height: mobileH * 30 / 100, borderRadius: 5, backgroundColor: Colors.imagebackcolor, position: "relative", top: 8 }} />
                                                    </TouchableOpacity>}
                                            ...
                                            {item.userid == this.state.user_id && <View style={{ width: '70%', alignSelf: 'flex-end', alignItems: 'flex-end', }}>
                                                <View style={{
                                                    ...
                                                    borderTopStartRadius: 6, borderTopEndRadius: 6, borderBottomLeftRadius: 6, paddingVertical: 1.5, paddingHorizontal: 8, alignSelf: 'flex-end',
                                                }}>
                        ...
                        {this.state.block_status == 'no' &&
                            <View style={{
                                ...
                                bottom: config.device_type == 'ios' ? this.state.bottom : 0,
                                flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.whiteColor,
                                justifyContent: 'space-between', position: 'absolute', borderTopWidth: 0.8,
                                ...
                                <TouchableOpacity
                                    ...
                                    style={{
                                        ...
                                        width: mobileW * 15 / 100, height: mobileH * 8 / 100,
                                        backgroundColor: Colors.whiteColor, borderTopWidth: 0.8,
                                        borderRightWidth: 0.8, borderColor: '#e2e2e2',
                                ...
                                <TouchableOpacity
                                    ...
                                    style={{
                                        alignItems: 'center', justifyContent: 'center',
                                        width: mobileW * 15 / 100, height: mobileH * 8 / 100,
                                        backgroundColor: Colors.whiteColor, borderLeftWidth: 0.8,
                                        borderTopWidth: 0.8,
                                        borderColor: '#e2e2e2'
                                    }}
                                >
